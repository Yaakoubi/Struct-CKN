...
class CKN(nn.Module):
    ...
    def train_network_sup(self, X, letters_y_train, clf, scaler, struct_model=None, X_train_words=None,
        ...
        if verbose:
            print("N = ", N)
        expected_output = Variable(torch.LongTensor(
            letters_y_train), requires_grad=False)
        if use_cuda:
        ...
        if init is True and opt is None:
            if self.Kernel[0].optimizer is not None:
                ...
                optimizer = optim.Adamax(
                    model_parameters, lr=self.Kernel[k].lr)
        else:
            optimizer = opt
        if verbose:
        ...
        for t in range(numberEpochsCKN):
            ...
            if save_checkpoint:
                self.save_checkpoint(args.time_stamp)
        output_map = self.propagate_through_network(
    ...
    def save_checkpoint(self, time_stamp):
        ...
        tuples = [a for a in attributes if (
