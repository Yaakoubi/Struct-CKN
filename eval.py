...
try:
    ...
    optweights = DenseWeights(bias = bias , transition = transition , emission = emission , nb_labels = 26 , nb_features = 7200 )
    optmargs = np.load("pretrained/optmarginals.npy.npz")
except: raise Exception("Files in ./pretrained/ not found.\n"+
  "Please load the pretrained weights from the following link:\n"+
  "https://bit.ly/Struct-CKN")
if use_sdca4crf:
    scaled_data_for_test = CKN.propagate_sdca(X_test, letters_y_test, clf, scaler,None, X_test_words, y_test_words, lengths_test,use_scaler)
    X2_test, y2_test = write_mat_file(scaled_data_for_test, letters_y_test,lengths_test, train=False, write_file=False)																
    predicted_labels_test = predict_sdca(optweights, train=False, test=True, include_eos=False,xtest=X2_test, ytest=y2_test, marginals=optmargs, args=args)
    predicted_labels_test = synchonize_y(original=letters_y_test, new_y=predicted_labels_test)
    print("Test Acc : ", np.sum((np.array(predicted_labels_test == letters_y_test)) * 1), " / ", np.array(letters_y_test).shape[0])
